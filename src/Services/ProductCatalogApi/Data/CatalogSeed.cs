...
{
    // This class should be called after the Service Registeration in Startup.cs
    public class CatalogSeed
    {
        // Callable method from client side
        // This is an example of a method injection, instead of a construction injection
        // inside CatalogContext
        public static async Task SeedAsync(CatalogContext context)
        {
            // If there are no CatalogBrand items in the database
            if(!context.CatalogBrands.Any())
            {
            // First call CatalogBrand and CatalogType because CatalogItem 
            // needs foreign keys from those two classes
            context.CatalogBrands.AddRange(GetPreconfiguredCatalogBrands());
            // So data will persist in the database
            await context.SaveChangesAsync();
            // We need to check existence, otherwise there will be duplicate items
            }
        ...
        // Create seeding data without having any IDs
        // The ID is generated by EF and hilo algorithm
        ...
        static IEnumerable<CatalogItem> GetPreconfiguredCatalogItems()
        {
            ...
            {
                // Copied over the items to save time
